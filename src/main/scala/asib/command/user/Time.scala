...
class Time extends AbstractUserCommand {

	val helpMessage = "Fetches the local time usage - !time <location>"
	...
	def execute(username: String, channel: String, args: String) = {
		...
		if (weatherApiKey == "None" || weatherApiKey == "") {
			...
			val u = url("http://www.worldweatheronline.com/feed/tz.ashx?key=" + weatherApiKey +
				"&q=" + Uri.encode(args) + "&format=json")
			val json = http(u >- JsonParser.parse)

			val time     = (json \ "data" \ "time_zone" \ "localtime" values)
			val location = (json \ "data" \ "request" \ "query" values)

			Asib.sendMsg(channel, username + " The time in " + location + " is " + time)
		}
