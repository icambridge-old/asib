...
class PrivMsgTest extends FunSuite with AsibSetup {

	test("command reaches user command") {
		val sentMessage     = Message("PRIVMSG","asib","#icambridge","!test hello world")
		privMsgTrigger.handle(sentMessage)
		val commandInfo = mockUserCommand.lastCommandPayload
		val username    = commandInfo.get("username").getOrElse("")
		val args        = commandInfo.get("args").getOrElse("")
